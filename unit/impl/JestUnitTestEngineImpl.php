...
final class JestUnitTestEngineImpl
{
        ...
        chdir(
            join(
                ...
                [
                    ...
                    $this->getConfig("unit.engine.jest.root_dir", "")
                ]
            )
        );
    ...
    {
        $this->command = $this->getConfig("unit.engine.jest.jest_binary", "./node_modules/.bin/jest");
        $this->command = $this->command . ' test --env=jsdom --json';
    ...
    private function readCoverage($coverage_file)
    {
        ...
        foreach ($files as $file) {
            ...
            if ($any_line_covered) {
                ...
                $len = strlen($this->unitTestBase->getWorkingCopy()->getProjectRoot() . DIRECTORY_SEPARATOR);
